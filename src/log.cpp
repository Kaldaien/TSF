...
tsf_logger_s dll_log;


void
tsf_logger_s::close (void)
{
  if (fLog != NULL) {
    fflush (fLog);
    fclose (fLog);
...
bool
tsf_logger_s::init (const char* const szFileName,
...
void
tsf_logger_s::LogEx (bool                 _Timestamp,
...
void
tsf_logger_s::Log   (_In_z_ _Printf_format_string_
                     wchar_t const* const _Format, ...)
{
  va_list _ArgList;
